...
def create_kitti_coco_json_without_dontcare():
    # Paths
    ...
    output_path = "datasets/kitti_coco/annotations/train2017_no_misc.json"
    
    # KITTI categories (excluding Misc/DontCare)
    categories = [
    ...
    # Classes to skip (including DontCare and other KITTI classes)
    skip_classes = {"DontCare", "Misc", "Person", "Truck_trailer", "Van_trailer"}
    
    # Initialize COCO format structure
    coco_data = {
        "info": {
            "description": "KITTI Dataset in COCO format (without Misc/DontCare)",
            "version": "1.0",
            "year": 2024,
    ...
    for img_id, img_file in enumerate(image_files):
        ...
        image_has_valid_annotations = False
        ...
        if os.path.exists(label_path):
            ...
            for line in lines:
                ...
                # Skip DontCare and other unwanted classes
                if class_name in skip_classes:
                    continue
                
                # Skip if class not in our valid categories
                ...
                annotation_id += 1
                image_has_valid_annotations = True
        
        if image_has_valid_annotations:
    ...
    print(f"🗑️  Skipped classes: {', '.join(skip_classes)}")

if __name__ == "__main__":
    create_kitti_coco_json_without_dontcare()